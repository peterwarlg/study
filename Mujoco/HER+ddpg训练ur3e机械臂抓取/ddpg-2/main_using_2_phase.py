...
def main():
    ...
    hp['sample_her_transitions'] = make_sample_her_transitions(
        replay_strategy=hp['replay_strategy'], replay_k=4, reward_fun=hp['reward_fun'], ep=True)

    hp['sample_her_transitions_2'] = make_sample_her_transitions(
        replay_strategy=hp['replay_strategy'], replay_k=4, reward_fun=hp['reward_fun'], ep=False)
    ...
    if hp['mode'] == 'train':
        np.savetxt(
            ...
            '环境: ' + hp['env_name'] + '——' +
            'seed: ' + str(hp['seed']) + '——' +
            '线程数: ' + str(hp['num_workers']) + '——' +
            '是否ep: ' + str(hp['ep']) + '——' +
            'seed: ' + str(hp['seed']) + '——' +
            '网络节点数' + str(hp['hidden_size']) + '——' +
            'epochs: ' + str(hp['n_epochs']) + '——' +
            '备注_' + hp['sp_name'] +
            '.csv',
            scores, delimiter=',')
