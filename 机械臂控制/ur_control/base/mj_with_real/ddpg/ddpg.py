...
from base.mj_with_real.ddpg.model import Actor, Critic
from collections import OrderedDict
from base.mj_with_real.ddpg.utils import Normalizer
from base.mj_with_real.ddpg.replay_buffer import ReplayBuffer
...
class ddpgAgent(object):
    ...
    def load_checkpoint(self, path, name, isbackup=False):
        if not isbackup:
            self.actor_local.load_state_dict(torch.load( path + '/'+name+'_checkpoint_actor_her.pth'))
            self.actor_target.load_state_dict(self.actor_local.state_dict())
            self.critic_local.load_state_dict(torch.load( path + '/'+name+'_checkpoint_critic_her.pth'))
            self.critic_target.load_state_dict(self.critic_local.state_dict())
            self.obs_normalizer.load_normalizer(path + '/'+name+'_obs_normalizer.pth')
            self.goal_normalizer.load_normalizer(path + '/'+name+'_goal_normalizer.pth')
        else:
            self.actor_local.load_state_dict(torch.load(path + '/' + name + '_checkpoint_actor_her_back.pth'))
            self.actor_target.load_state_dict(self.actor_local.state_dict())
            self.critic_local.load_state_dict(torch.load(path + '/' + name + '_checkpoint_critic_her_back.pth'))
            self.critic_target.load_state_dict(self.critic_local.state_dict())
            self.obs_normalizer.load_normalizer(path + '/' + name + '_obs_normalizer_back.pth')
            self.goal_normalizer.load_normalizer(path + '/' + name + '_goal_normalizer_back.pth')
