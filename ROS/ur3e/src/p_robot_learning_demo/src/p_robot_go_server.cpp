    ...
    void ExecuteCb(const p_robot_learning_demo::UrDemoGoalConstPtr& goal)
    {
		...
		if (mode_id != 0)
		{
			if (mode_id == 1)
			{
				...
				switch (action_id)
				{
					...
					case 2: suc = acpl.move(&obj);
						break;
					case 3: suc = acpl.pick_obj(&obj);  // pick_obj=simu, pick_obj2=real
						break;
					case 4: suc = acpl.place_obj(&obj);  // place_obj=simu, place_obj2=real
						break;
					case 5: suc = acpl.gripper_grasp2(1);
						break;
					case 6: suc = acpl.gripper_grasp2(0);
						break;
					case 11: group.setNamedTarget("home_b_1");
						//suc = group.plan(my_plan);   //2020
						//suc = group.execute(my_plan);		//2020

						suc = (group.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
						suc = (group.execute(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);


						group.setNamedTarget("home_b_2");
						//suc = group.plan(my_plan);		//2020
						//suc = group.execute(my_plan);		//2020
						...
						break;
					case 12: group.setNamedTarget("home_b_2");
						//suc = group.plan(my_plan);		//2020
						//suc = group.execute(my_plan);		//2020
						suc = (group.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
						suc = (group.execute(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
			...
			else if (mode_id == 2)
			{
				...
				//bool success = group.plan(my_plan);		//2020
		...
		ROS_INFO("z= %lf .",current_pose.pose.position.z);
	        ROS_INFO("w= %lf .",current_pose.pose.orientation.w);
		ROS_INFO("wx= %lf .",current_pose.pose.orientation.x);
		ROS_INFO("wy= %lf .",current_pose.pose.orientation.y);
		ROS_INFO("wz= %lf .",current_pose.pose.orientation.z);
		...
		result.rurpos.push_back(current_pose.pose.orientation.z);
                result.rurpos.push_back(joint_values[0]);
                result.rurpos.push_back(joint_values[1]);
                result.rurpos.push_back(joint_values[2]);
                result.rurpos.push_back(joint_values[3]);
                result.rurpos.push_back(joint_values[4]);
                result.rurpos.push_back(joint_values[5]);
